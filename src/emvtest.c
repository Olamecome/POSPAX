
#include "global.h"


/************************************************
Belows are EMV test data
************************************************/

EMV_CAPK capk_visa_t89 = { { 0xA0,0x00,0x00,0x00,0x03 },0x89,
0x01,0x01,192,
{  },
1,
{ 0x03 },
{ 0x25,0x12,0x31 },
{ 0x71, 0x70, 0x85, 0x0B, 0x97, 0xF8, 0x39, 0x52, 0x04, 0x5C, 0xF9, 0xCA, 0x8B, 0x76, 0x12, 0xDF,
0xEB, 0x69, 0xE9, 0xEF }
};

// VISA 1408 bits Test Key 92
EMV_CAPK capk_visa_t92 = { {0xA0,0x00,0x00,0x00,0x03},0x92,
0x01,0x01,176,
{},
1,{0x03},{0x25,0x12,0x31},
{0x42,0x9C,0x95,0x4A,0x38,0x59,0xCE,0xF9,0x12,0x95,0xF6,0x63,0xC9,0x63,0xE5,0x82,0xED,
0x6E,0xB2,0x53}
};

// VISA 1984 bits Test Key 94 20080328
EMV_CAPK capk_visa_t94 = { {0xA0,0x00,0x00,0x00,0x03},0x94,
0x01,0x01,248,
{},
1,{0x03},{0x25,0x12,0x31},
{0xC4,0xA3,0xC4,0x3C,0xCF,0x87,0x32,0x7D,0x13,0x6B,0x80,0x41,0x60,0xE4,0x7D,0x43,
0xB6,0x0E,0x6E,0x0F}
};


// VISA 1152 bits Test Key 95
EMV_CAPK capk_visa_t95 = { {0xA0,0x00,0x00,0x00,0x03},0x95,
0x01,0x01,144,
{},
1,{0x03},{0x25,0x12,0x31},
{0xEE,0x15,0x11,0xCE,0xC7,0x10,0x20,0xA9,0xB9,0x04,0x43,0xB3,0x7B,0x1D,0x5F,0x6E,
0x70,0x30,0x30,0xF6}
};

// VISA 1024 bits Test Key 99
EMV_CAPK capk_visa_t99 = { {0xA0,0x00,0x00,0x00,0x03},0x99,
0x01,0x01,128,
{},
1,{0x03},{0x25,0x12,0x31},
{0x4A,0xBF,0xFD,0x6B,0x1C,0x51,0x21,0x2D,0x05,0x55,0x2E,0x43,0x1C,0x5B,0x17,0x00,
0x7D,0x2F,0x5E,0x6D}
};

// Mastercard 1984 bits transaction time test Key EF
EMV_CAPK capk_mastercard_tEF = { {0xA0,0x00,0x00,0x00,0x04},0xEF,
0x01,0x01,248,
{ 
},
1,{0x03},{0x25,0x12,0x31},
{0x21,0x76,0x6E,0xBB,0x0E,0xE1,0x22,0xAF,0xB6,0x5D,0x78,0x45,0xB7,0x3D,0xB4,0x6B,
0xAB,0x65,0x42,0x7A}
};

// Mastercard 1408 bits transaction time test Key F1
EMV_CAPK capk_mastercard_tF1 = { {0xA0,0x00,0x00,0x00,0x04},0xF1,
0x01,0x01,176,
{},
1,{0x03},{0x25,0x12,0x31},
{0xD8,0xE6,0x8D,0xA1,0x67,0xAB,0x5A,0x85,0xD8,0xC3,0xD5,0x5E,0xCB,0x9B,0x05,0x17,
0xA1,0xA5,0xB4,0xBB}
};

// Mastercard 1152 bits transaction time test Key F2 (exponent=65537)
EMV_CAPK capk_mastercard_tF2 = { {0xA0,0x00,0x00,0x00,0x04},0xF2,
0x01,0x01,144,
{},
3,{0x01,0x00,0x01},{0x25,0x12,0x31},
{0x0F,0x3B,0xA8,0xCB,0x57,0x77,0xDC,0x4A,0xA9,0x6C,0x30,0xBF,0xB1,0xFC,0x26,0x7A,
0x38,0x2A,0x48,0x47}
};

// Mastercard 1152 bits transaction time test Key F3
EMV_CAPK capk_mastercard_tF3 = { {0xA0,0x00,0x00,0x00,0x04},0xF3,
0x01,0x01,144,
{},
1,{0x03},{0x25,0x12,0x31},
{0xA6,0x9A,0xC7,0x60,0x3D,0xAF,0x56,0x6E,0x97,0x2D,0xED,0xC2,0xCB,0x43,0x3E,0x07,
0xE8,0xB0,0x1A,0x9A}
};

// Mastercard 896 bits transaction time test Key F5
EMV_CAPK capk_mastercard_tF5 = { {0xA0,0x00,0x00,0x00,0x04},0xF5,
0x01,0x01,248,
{ },
3,{ 0x01, 0x00, 0x01 },{ 0x22,0x12,0x31 },
{ 0xC2, 0x23, 0x98, 0x04, 0xC8, 0x09, 0x81, 0x70, 0xBE, 0x52, 0xD6, 0xD5, 0xD4, 0x15, 0x9E, 0x81,
0xCE, 0x84, 0x66, 0xBF} };

EMV_CAPK capk_mastercard_tF6 = { { 0xA0,0x00,0x00,0x00,0x04 },0xF6,
0x01,0x01,224,
{ 
},
1,
{ 0x03 },
{ 0x25,0x12,0x31 },
{ 0x50, 0x29, 0x09, 0xED, 0x54, 0x5E, 0x3C, 0x8D, 0xBD, 0x00, 0xEA, 0x58, 0x2D, 0x06, 0x17, 0xFE,
0xE9, 0xF6, 0xF6, 0x84} };

EMV_CAPK capk_mastercard_tF7 = { { 0xA0,0x00,0x00,0x00,0x04 },0xF7,
0x01,0x01,128,
{ 
},
3,
{ 0x01, 0x00, 0x01 },
{ 0x25,0x12,0x31 },
{ 0xEE, 0xB0, 0xDD, 0x9B, 0x24, 0x77, 0xBE, 0xE3, 0x20, 0x9A, 0x91, 0x4C, 0xDB, 0xA9, 0x4C, 0x1C,
0x4A, 0x9B, 0xDE, 0xD9
}
};

// Mastercard 1024 bits transaction time test Key F8 (also for ETEC test)
EMV_CAPK capk_mastercard_tF8 = { {0xA0,0x00,0x00,0x00,0x04},0xF8,
0x01,0x01,128,
{},
1,{0x03},{0x25,0x12,0x31},
{0xF0,0x6E,0xCC,0x6D,0x2A,0xAE,0xBF,0x25,0x9B,0x7E,0x75,0x5A,0x38,0xD9,0xA9,0xB2,
0x4E,0x2F,0xF3,0xDD}
};

EMV_CAPK capk_mastercard_tF9 = { { 0xA0,0x00,0x00,0x00,0x04 },0xF9,
0x01,0x01,192,
{ 
},
1,
{ 0x03 },
{ 0x25,0x12,0x31 },
{ 0x33, 0x67, 0x12, 0xDC, 0xC2, 0x85, 0x54, 0x80, 0x9C, 0x6A, 0xA9, 0xB0, 0x23, 0x58, 0xDE, 0x6F,
0x75, 0x51, 0x64, 0xDB }
};


EMV_CAPK capk_mastercard_tFA = { {0xA0,0x00,0x00,0x00,0x04},0xFA,
0x01,0x01,144,
{},
1,{0x03},{0x25,0x12,0x31},
{0x5B,0xED,0x40,0x68,0xD9,0x6E,0xA1,0x6D,0x2D,0x77,0xE0,0x3D,0x60,0x36,0xFC,0x7A,
0x16,0x0E,0xA9,0x9C}
};

// Mastercard 1024 bits Test Key FE(also for ETEC test)
EMV_CAPK capk_mastercard_tFE = { {0xA0,0x00,0x00,0x00,0x04},0xFE,
0x01,0x01,128,
{},
1,{0x03},{0x25,0x12,0x31},
{0x9A,0x29,0x5B,0x05,0xFB,0x39,0x0E,0xF7,0x92,0x3F,0x57,0x61,0x8A,0x9F,0xDA,0x29,
0x41,0xFC,0x34,0xE0}
};

EMV_CAPK capk_mastercard_v00 = {
	{ 0xA0,0x00,0x00,0x00,0x04 },
	0x00,
	0x01,0x01,
	160,
	{  },
	1,
	{ 0x03 },
	{ 0x25,0x12,0x31 },
	{ 0xEC, 0x0A, 0x59, 0xD3, 0x5D, 0x19, 0xF0, 0x31, 0xE9, 0xE8, 0xCB, 0xEC, 0x56, 0xDB, 0x80, 0xE2,
	0x2B, 0x1D, 0xE1, 0x30}
};

EMV_CAPK capk_mastercard_v01 = {
	{ 0xA0,0x00,0x00,0x00,0x04 },
	0x01,
	0x01,0x01,
	128,
	{ 
	},
	1,
	{ 0x03 },
	{ 0x25,0x12,0x31 },
	{ 0x8C, 0x05, 0xA6, 0x41, 0x27, 0x48, 0x5B, 0x92, 0x3C, 0x94, 0xB6, 0x3D, 0x26, 0x4A, 0xF0, 0xBF,
	0x85, 0xCB, 0x45, 0xD9 }
};

EMV_CAPK capk_mastercard_v02 = {
	{ 0xA0,0x00,0x00,0x00,0x04 },
	0x02,
	0x01,0x01,
	192,
	{ 
	},
	1,
	{ 0x03 },
	{ 0x25,0x12,0x31 },
	{ 0x33, 0x40, 0x8B, 0x96, 0xC8, 0x14, 0x74, 0x2A, 0xD7, 0x35, 0x36, 0xC7, 0x2F, 0x09, 0x26, 0xE4,
	0x47, 0x1E, 0x8E, 0x47
	}
};

EMV_CAPK capk_mastercard_v05_test = {
	{ 0xA0,0x00,0x00,0x00,0x04 },
	0x05,
	0x01,0x01,
	128,
	{ 
	},
	1,
	{ 0x03 },
	{ 0x25,0x12,0x31 },
	{ 0x53, 0xD0, 0x49, 0x03, 0xB4, 0x96, 0xF5, 0x95, 0x44, 0xA8, 0x43, 0x09, 0xAF, 0x16, 0x92, 0x51,
	0xF2, 0x89, 0x68, 0x74 }
};

// JCB 1024 bits Test Key 08
EMV_CAPK capk_jcb_t08 = { {0xA0,0x00,0x00,0x00,0x65},0x08,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0xDD,0x36,0xD5,0x89,0x62,0x28,0xC8,0xC4,0x90,0x07,0x42,0xF1,0x07,0xE2,0xF9,0x1F,
0xE5,0x0B,0xC7,0xEE}
};

// JCB 1024 bits Test Key 01
EMV_CAPK capk_jcb_t01 = { {0xF1,0x23,0x45,0x67,0x89},0x01,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0xCC,0xC1,0x8B,0x93,0x7C,0x85,0x9F,0x42,0x9C,0x33,0xA3,0xDB,0xEB,0x45,0xE3,0x1E,
0x17,0x5B,0x8F,0xD1}
};

// JCB 1024 bits Test Key 02,for JCB2.0 test
EMV_CAPK capk_jcb_t02 = { {0xF1,0x23,0x45,0x67,0x89},0x02,
0x01,0x01,128,
{},
3,{0x01,0x00,0x01},{0x15,0x12,0x31},
{0x5A,0x52,0xA2,0x86,0x9A,0x93,0x8F,0x82,0xBE,0x41,0xA1,0x80,0xA3,0x98,0x09,0x55,
0x46,0xD4,0xF6,0xAA}
};

// JCB 1152 bits Test Key 03,for JCB2.0 test
EMV_CAPK capk_jcb_t03 = { {0xF1,0x23,0x45,0x67,0x89},0x03,
0x01,0x01,144,
{},
1,{0x03},{0x15,0x12,0x31},
{0xE9,0x6C,0x1B,0x32,0xB4,0xBA,0xEE,0xDA,0x02,0x99,0x91,0x51,0x42,0x23,0x63,0x72,
0xFF,0xC6,0x8A,0x64}
};

// for JCB test [7/31/2006 Tommy] ---->
EMV_CAPK capk_jcb_tDA = { {0xA0,0x00,0x00,0x00,0x65},0xDA,
0x01,0x01,128,
{},
1,{0x03}, {0x10,0x10,0x31},
{0x5C,0x03,0x7E,0x18,0x15,0xE3,0x8B,0x79,0xD4,0xD5,0xC1,0x5F,0xF9,0x0C,0xE4,0xE3,0xF2,
0x07,0x08,0x63}
};

EMV_CAPK capk_jcb_tEA = { {0xA0,0x00,0x00,0x00,0x65},0xEA,
0x01,0x01,144,
{},
1,{0x03}, {0x10,0x12,0x31},
{0x88,0xF6,0xD0,0x4B,0xED,0x35,0xD6,0xB4,0x3E,0x69,0x7D,0x76,0x5A,0xE3,0x29,0x3F,0x64,
0x9A,0x96,0x1F}
};

EMV_CAPK capk_jcb_tEB = { {0xA0,0x00,0x00,0x00,0x65},0xEB,
0x01,0x01,176,
{},
1,{0x03},{0x10,0x10,0x31},
{0xA1,0x9C,0xAF,0x70,0x06,0x03,0xE3,0x7B,0xC7,0xC2,0xE5,0xC4,0x20,0xA1,0x1A,0x56,0xD1,
0xA8,0xBA,0xC3}
};

EMV_CAPK capk_jcb_tEC = { {0xA0,0x00,0x00,0x00,0x65},0xEC,
0x01,0x01,248,
{},
1,{0x03},{0x10,0x10,0x31},
{0xA1,0x59,0x45,0x94,0x69,0x35,0x95,0x68,0x45,0xAD,0xB8,0xAB,0xE7,0x3E,0x5B,0x0B,0xEE,
0xF7,0x6E,0xCB}
};

EMV_CAPK capk_jcb_t0F = { {0xA0,0x00,0x00,0x00,0x65},0x0F,
0x01,0x01,144,
{/*0x9F*/0x9E,},
1,{0x03},{0x10,0x12,0x31},
{0x2A,0x1B,0x82,0xDE,0x00,0xF5,0xF0,0xC4,0x01,0x76,0x0A,0xDF,0x52,0x82,0x28,0xD3,0xED,
0xE0,0xF4,0x03}
};
// for JCB test [7/31/2006 Tommy] <----

EMV_CAPK capk_jcb_t09 = { {0xA0,0x00,0x00,0x00,0x65},0x09,
0x01,0x01,128,
{},
1,{0x03},{0x09,0x12,0x31},
{0x44,0x10,0xC6,0xD5,0x1C,0x2F,0x83,0xAD,0xFD,0x92,0x52,0x8F,0xA6,0xE3,0x8A,0x32,0xDF,
0x04,0x8D,0x0A}
};

EMV_CAPK capk_jcb_t10 = { {0xA0,0x00,0x00,0x00,0x65},0x10,
0x01,0x01,144,
{},
1,{0x03},{0x12,0x12,0x31},
{0xC7,0x5E,0x52,0x10,0xCB,0xE6,0xE8,0xF0,0x59,0x4A,0x0F,0x19,0x11,0xB0,0x74,0x18,
0xCA,0xDB,0x5B,0xAB}
};

EMV_CAPK capk_jcb_t11 = { {0xA0,0x00,0x00,0x00,0x65},0x11,
0x01,0x01,176,
{},
1,{0x03},{0x12,0x12,0x31},
{0xD9,0xFD,0x62,0xC9,0xDD,0x4E,0x6D,0xE7,0x74,0x1E,0x9A,0x17,0xFB,0x1F,0xF2,0xC5,0xDB,
0x94,0x8B,0xCB}
};

EMV_CAPK capk_jcb_t12 = { {0xA0,0x00,0x00,0x00,0x65},0x12,
0x01,0x01,176,
{},
1,{0x03},{0x14,0x12,0x31},
{0x87,0x4B,0x37,0x9B,0x7F,0x60,0x7D,0xC1,0xCA,0xF8,0x7A,0x19,0xE4,0x00,0xB6,0xA9,
0xE2,0x51,0x63,0xE8}
};

EMV_CAPK capk_jcb_t13 = { {0xA0,0x00,0x00,0x00,0x65},0x13,
0x01,0x01,248,
{},
1,{0x03},{0x14,0x12,0x31},
{0x54,0xCF,0xAE,0x61,0x71,0x50,0xDF,0xA0,0x9D,0x3F,0x90,0x1C,0x91,0x23,0x52,0x45,0x23,
0xEB,0xED,0xF3}
};

EMV_CAPK capk_jcb_t14 = { {0xA0,0x00,0x00,0x00,0x65},0x14,
0x01,0x01,248,
{},
1,{0x03},{0x16,0x12,0x31},
{0xC0,0xD1,0x5F,0x6C,0xD9,0x57,0xE4,0x91,0xDB,0x56,0xDC,0xDD,0x1C,0xA8,0x7A,0x03,
0xEB,0xE0,0x6B,0x7B}
};

EMV_CAPK capk_amex_test62 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x62,
0x01, 0x01, 0x60,
{0xBA, 0x29, 0xDE, 0x83, 0x09, 0x0D, 0x8D, 0x5F, 0x4D, 0xFF, 0xCE, 0xB9, 0x89, 0x18, 0x99, 0x5A,
0x76, 0x8F, 0x41, 0xD0, 0x18, 0x3E, 0x1A, 0xCA, 0x3E, 0xF8, 0xD5, 0xED, 0x90, 0x62, 0x85, 0x3E,
0x40, 0x80, 0xE0, 0xD2, 0x89, 0xA5, 0xCE, 0xDD, 0x4D, 0xD9, 0x6B, 0x1F, 0xEA, 0x2C, 0x53, 0x42,
0x84, 0x36, 0xCE, 0x15, 0xA2, 0xA1, 0xBF, 0xE6, 0x9D, 0x46, 0x19, 0x7D, 0x3F, 0x5A, 0x79, 0xBC,
0xF8, 0xF4, 0x85, 0x8B, 0xFF, 0xA0, 0x4E, 0xDB, 0x07, 0xFC, 0x5B, 0xE8, 0x56, 0x0D, 0x9C, 0xE3,
0x8F, 0x5C, 0x3C, 0xA3, 0xC7, 0x42, 0xED, 0xFD, 0xBA, 0xE3, 0xB5, 0xE6, 0xDD, 0xA4, 0x55, 0x57},
1,{0x03},{0x15, 0x12, 0x31},
{0xCC, 0xC7, 0x30, 0x3F, 0xF2, 0x95, 0xA9, 0xF3, 0x5B, 0xA6, 0x1B, 0xD3, 0x1E, 0x27, 0xEA, 0xBD,
0x59, 0x65, 0x82, 0x65}
};

EMV_CAPK capk_amex_test64 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x64,
0x01, 0x01, 0x80,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0x79, 0x2B, 0x12, 0x1D, 0x86, 0xD0, 0xF3, 0xA9, 0x95, 0x82, 0xDB, 0x06, 0x97, 0x44, 0x81, 0xF3,
0xB2, 0xE1, 0x84, 0x54}
};

EMV_CAPK capk_amex_test65 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x65,
0x01, 0x01, 0x90,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0x89, 0x4C, 0x5D, 0x08, 0xD4, 0xEA, 0x28, 0xBB, 0x79, 0xDC, 0x46, 0xCE, 0xAD, 0x99, 0x8B, 0x87,
 0x73, 0x22, 0xF4, 0x16}
};

EMV_CAPK capk_amex_test66 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x66,
0x01, 0x01, 0x80,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xF3, 0x67, 0xCB, 0x70, 0xF9, 0xC9, 0xB6, 0x7B, 0x58, 0x0F, 0x53, 0x38, 0x19, 0xE3, 0x02, 0xBA,
 0xC0, 0x33, 0x00, 0x90}
};

EMV_CAPK capk_amex_test04 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x04,
0x01, 0x01, 0x60,
{0xD0, 0xF5, 0x43, 0xF0, 0x3F, 0x25, 0x17, 0x13, 0x3E, 0xF2, 0xBA, 0x4A, 0x11, 0x04, 0x48, 0x67,
0x58, 0x63, 0x0D, 0xCF, 0xE3, 0xA8, 0x83, 0xC7, 0x7B, 0x4E, 0x48, 0x44, 0xE3, 0x9A, 0x9B, 0xD6,
0x36, 0x0D, 0x23, 0xE6, 0x64, 0x4E, 0x1E, 0x07, 0x1F, 0x19, 0x6D, 0xDF, 0x2E, 0x4A, 0x68, 0xB4,
0xA3, 0xD9, 0x3D, 0x14, 0x26, 0x8D, 0x72, 0x40, 0xF6, 0xA1, 0x4F, 0x0D, 0x71, 0x4C, 0x17, 0x82,
0x7D, 0x27, 0x9D, 0x19, 0x2E, 0x88, 0x93, 0x1A, 0xF7, 0x30, 0x07, 0x27, 0xAE, 0x9D, 0xA8, 0x0A,
0x3F, 0x0E, 0x36, 0x6A, 0xEB, 0xA6, 0x17, 0x78, 0x17, 0x17, 0x37, 0x98, 0x9E, 0x1E, 0xE3, 0x09},
1,{0x03},{0x15, 0x12, 0x31},
{0xFD, 0xD7, 0x13, 0x9E, 0xC7, 0xE0, 0xC3, 0x31, 0x67, 0xFD, 0x61, 0xAD, 0x3C, 0xAD, 0xBD,
0x68, 0xD6, 0x6E, 0x91, 0xC5}
};

EMV_CAPK capk_amex_testC7 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0xC7,
0x01, 0x01, 144,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0x62, 0x21, 0xE0, 0xC7, 0x26, 0xBA, 0xC8, 0xF8, 0xAC, 0x25, 0xF8, 0xF9, 0x3B, 0x81, 0x1D, 0x1F,
0xFD, 0x4C, 0x13, 0x1C}
};

EMV_CAPK capk_amex_testC8 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0xC8,
0x01, 0x01, 0x90,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0x33, 0xBD, 0x7A, 0x05, 0x9F, 0xAB, 0x09, 0x49, 0x39, 0xB9, 0x0A, 0x8F, 0x35, 0x84, 0x5C, 0x9D,
0xC7, 0x79, 0xBD, 0x50}
};

EMV_CAPK capk_amex_testC9 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0xC9,
0x01, 0x01, 176,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0x8E, 0x8D, 0xFF, 0x44, 0x3D, 0x78, 0xCD, 0x91, 0xDE, 0x88, 0x82, 0x1D, 0x70, 0xC9, 0x8F, 0x06,
0x38, 0xE5, 0x1E, 0x49}
};

EMV_CAPK capk_amex_testCA = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0xCA,
0x01, 0x01, 248,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0x6B, 0xDA, 0x32, 0xB1, 0xAA, 0x17, 0x14, 0x44, 0xC7, 0xE8, 0xF8, 0x80, 0x75, 0xA7, 0x4F, 0xBF,
0xE8, 0x45, 0x76, 0x5F}
};

EMV_CAPK capk_amex_test97 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x97,
0x01, 0x01, 144,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0xEB, 0xDA, 0x52, 0x2B, 0x63, 0x1B, 0x3E, 0xB4, 0xF4, 0xCB, 0xFC, 0x06, 0x79, 0xC4, 0x50, 0x13,
0x9D, 0x2B, 0x69, 0xCD}
};

EMV_CAPK capk_amex_test98 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x98,
0x01, 0x01, 176,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0xD4, 0xDB, 0xA4, 0x28, 0xCF, 0x11, 0xD4, 0x5B, 0xAE, 0xB0, 0xA3, 0x5C, 0xAE, 0xA8, 0x00, 0x7A,
0xD8, 0xBA, 0x8D, 0x71}
};

EMV_CAPK capk_amex_test99 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x99,
0x01, 0x01, 248,
{},
1,{0x03},{0x20, 0x12, 0x31},
{0x94, 0x79, 0x0D, 0x02, 0x0F, 0x4F, 0x69, 0x2D, 0x59, 0x28, 0x9F, 0x36, 0x45, 0x18, 0x72, 0x07,
0x80, 0x05, 0xB6, 0x3B}
};


//DPAS  bits Live Key 01
EMV_CAPK capk_dpas_test01 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x01,
0x01, 0x01,128,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xB0, 0x80, 0x31, 0xBD, 0xA9, 0xC3, 0x29, 0x1A, 0x3E, 0x8B, 0x9C, 0x79, 0x39, 0x15, 0xF2, 0x47,
0xA3, 0x84, 0x6B, 0x8F}
};


//DPAS  bits Live Key 02
EMV_CAPK capk_dpas_test02 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x02,
0x01, 0x01,160,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xF2, 0x3C, 0xE5, 0xC6, 0x57, 0xF6, 0xEC, 0x44, 0xFA, 0xD8, 0x25, 0x82, 0x35, 0x53, 0x0B, 0xC1,
0x13, 0x75, 0xE9, 0xE1}
};


//DPAS  bits Live Key 03
EMV_CAPK capk_dpas_test03 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x03,
0x01, 0x01, 128,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xB9, 0x6E, 0x38, 0x53, 0x33, 0x77, 0xDD, 0x43, 0x4C, 0x6B, 0x5E, 0xBB, 0xA3, 0x3B, 0x61, 0xDA,
0xD9, 0xA0, 0x4D, 0x51}
};


//DPAS  bits Live Key 04
EMV_CAPK capk_dpas_test04 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x04,
0x01, 0x01, 176,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xD9, 0xFD, 0x62, 0xC9, 0xDD, 0x4E, 0x6D, 0xE7, 0x74, 0x1E, 0x9A, 0x17, 0xFB, 0x1F, 0xF2, 0xC5,
0xDB, 0x94, 0x8B, 0xCB}
};


//DPAS  bits Live Key 5A
EMV_CAPK capk_dpas_test5A = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5A,
0x01, 0x01, 128,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0xCC, 0x95, 0x85, 0xE8, 0xE6, 0x37, 0x19, 0x1C, 0x10, 0xFC, 0xEC, 0xB3, 0x2B, 0x5A, 0xE1, 0xB9,
0xD4, 0x10, 0xB5, 0x2D}
};

//DPAS  Live Key 5B
EMV_CAPK capk_dpas_test5B = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5B,
0x01, 0x01, 144,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0x4D, 0xC5, 0xC6, 0xCA, 0xB6, 0xAE, 0x96, 0x97, 0x4D, 0x9D, 0xC8, 0xB2, 0x43, 0x5E, 0x21, 0xF5,
0x26, 0xBC, 0x7A, 0x60}
};


//DPAS  Live Key 5C
EMV_CAPK capk_dpas_test5C = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5C,
0x01, 0x01, 176,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0x60, 0x15, 0x40, 0x98, 0xCB, 0xBA, 0x35, 0x0F, 0x5F, 0x48, 0x6C, 0xA3, 0x10, 0x83, 0xD1, 0xFC,
0x47, 0x4E, 0x31, 0xF8}
};

//==========================================VERVE=====================================
EMV_CAPK verve_capk_v06 = {
	{ 0xA0, 0x00, 0x00, 0x03, 0x71 },
	0x06,0x01, 0x01, 247,
	{ 
	},
	1,
	{ 0X03 },
	{ 0x28, 0x12, 0x31 },
	{ 0xF5, 0xBA, 0xB8, 0x4E, 0xCE, 0x5F, 0x8B, 0xD4, 0x55, 0x11, 0xE5, 0xCA, 0x86, 0x1B, 0x80, 0xC7,
	0xE6, 0xC5, 0x1F, 0x55 }
};
EMV_CAPK verve_capk_v05 = {
	{ 0xA0, 0x00, 0x00, 0x03, 0x71 },
	0x05,0x01, 0x01, 176,
	{  },
	1,
	{ 0X03 },
	{ 0x24, 0x12, 0x31 },
	{ 0x67, 0x68, 0x22, 0xD3, 0x35, 0xAB, 0x0D, 0x2C, 0x38, 0x48, 0x41, 0x8C, 0xB5, 0x46, 0xDF, 0x7B,
	0x6A, 0x6C, 0x32, 0xC0 }
};

struct {
	unsigned char KeyID;
	EMV_CAPK *KeyValue;
}AllTestEmvKey[] = {
	{0x01,&capk_dpas_test01},//for DPAS test[2015/12/17 Gillian]
	{0x02,&capk_dpas_test02},
	{0x03,&capk_dpas_test03},
	{0x04,&capk_dpas_test04},
	{0x5A,&capk_dpas_test5A},
	{0x5B,&capk_dpas_test5B},
	{0x5C,&capk_dpas_test5C},

	{0x89,&capk_visa_t89},
	{0x92,&capk_visa_t92},
	{0x94,&capk_visa_t94},
	{0x95,&capk_visa_t95},
	{0x99,&capk_visa_t99},

	{ 0x00,&capk_mastercard_v00 },
	{ 0x01,&capk_mastercard_v01 },
	{ 0x02,&capk_mastercard_v02 },
	{ 0x05,&capk_mastercard_v05_test },
	{0xF1,&capk_mastercard_tF1},
	{0xF2,&capk_mastercard_tF2},
	{0xF3,&capk_mastercard_tF3},
	{0xF5,&capk_mastercard_tF5 },
	{0xF6,&capk_mastercard_tF6 },
	{0xF7,&capk_mastercard_tF7 },
	{0xF8,&capk_mastercard_tF8},
	{0xF9,&capk_mastercard_tF9 },
	{0xFA,&capk_mastercard_tFA},
	{0xFE,&capk_mastercard_tFE},
	{ 0xEF,&capk_mastercard_tEF },

	{0xEB,&capk_jcb_tEB},
	{0xEC,&capk_jcb_tEC},
	{0x0f,&capk_jcb_t0F},
	{0x11,&capk_jcb_t11},
	{0x13,&capk_jcb_t13},
	{0xDA,&capk_jcb_tDA},
	{0xEA,&capk_jcb_tEA},

	{0x04,&capk_amex_test04},
	{0x62,&capk_amex_test62},
	{0x64,&capk_amex_test64},
	{0x65,&capk_amex_test65},
	{0x66,&capk_amex_test66},
	{0xC7,&capk_amex_testC7},
	{0xC8,&capk_amex_testC8},
	{0xC9,&capk_amex_testC9},
	{0xCA,&capk_amex_testCA},
	{ 0x05, &verve_capk_v05 },
	{ 0x06, &verve_capk_v06 }
	//	{0x67,&capk_amex_test67},
	//	{0x68,&capk_amex_test68},


};

/************************************************
End of EMV test data
************************************************/








/************************************************
Belows are production EMV data
************************************************/

// VISA 1408 BITS LIVE KEY 08
EMV_CAPK capk_visa_v08 = { {0xA0,0x00,0x00,0x00,0x03},0x08,
0x01,0x01,176,
{  },
1,{0x03}, {0x24,0x12,0x31},
{ 0x20, 0xD2, 0x13, 0x12, 0x69, 0x55, 0xDE, 0x20, 0x5A, 0xDC, 0x2F, 0xD2, 0x82, 0x2B, 0xD2, 0x2D,
0xE2, 0x1C, 0xF9, 0xA8}
};

// VISA 1984 BITS LIVE KEY 09
EMV_CAPK capk_visa_v09 = { {0xA0,0x00,0x00,0x00,0x03},0x09,
0x01,0x01,248,
{ 
},
1,{0x03}, {0x28,0x12,0x31},
{ 0x1F, 0xF8, 0x0A, 0x40, 0x17, 0x3F, 0x52, 0xD7, 0xD2, 0x7E, 0x0F, 0x26, 0xA1, 0x46, 0xA1, 0xC8,
0xCC, 0xB2, 0x90, 0x46}
};


// Mastercard 1408 bits live Key 05
EMV_CAPK capk_mastercard_v05 = { {0xA0,0x00,0x00,0x00,0x04},0x05,
0x01,0x01,176,
{ 
},
1,{0x03},{0x24,0x12,0x31},
{ 0xEB, 0xFA, 0x0D, 0x5D, 0x06, 0xD8, 0xCE, 0x70, 0x2D, 0xA3, 0xEA, 0xE8, 0x90, 0x70, 0x1D, 0x45,
0xE2, 0x74, 0xC8, 0x45 }
};

// Mastercard 1948 bits live Key 06
EMV_CAPK capk_mastercard_v06 = { {0xA0,0x00,0x00,0x00,0x04},0x06,
0x01,0x01,248,
{ },
1,{0x03},{0x28,0x12,0x31},
{ 0xF9, 0x10, 0xA1, 0x50, 0x4D, 0x5F, 0xFB, 0x79, 0x3D, 0x94, 0xF3, 0xB5, 0x00, 0x76, 0x5E, 0x1A,
0xBC, 0xAD, 0x72, 0xD9}
};



EMV_CAPK capk_amex_live0F = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x0F,
0x01, 0x01,176,
{},
1,{0x03},{0x24, 0x12, 0x31},
{0xA7, 0x34, 0x72, 0xB3, 0xAB, 0x55, 0x74, 0x93, 0xA9, 0xBC, 0x21, 0x79, 0xCC, 0x80, 0x14, 0x05,
0x3B, 0x12, 0xBA, 0xB4}
};

//AMEX 1984 bits Live Key 10
EMV_CAPK capk_amex_live10 = { {0xA0, 0x00, 0x00, 0x00, 0x25}, 0x10,
0x01, 0x01,248,
{},
1,{0x03},{0x27, 0x12, 0x31},
{0xC7, 0x29, 0xCF, 0x2F, 0xD2, 0x62, 0x39, 0x4A, 0xBC, 0x4C, 0xC1, 0x73, 0x50, 0x65, 0x02, 0x44,
0x6A, 0xA9, 0xB9, 0xFD}
};

//AMEX 1984 bits Live Key 67
EMV_CAPK capk_amex_live67 = { { 0xA0, 0x00, 0x00, 0x00, 0x25 }, 0x67,
0x01, 0x01,0xB0,
{  },
1,{ 0x03 },{ 0x22, 0x12, 0x31 },
{ 0x52, 0xA2, 0x90, 0x73, 0x00, 0xC8, 0x44, 0x5B, 0xF5, 0x4B, 0x97, 0x0C, 0x89, 0x46, 0x91,0xFE, 0xAD, 0xF2, 0xD2, 0x8E }
};

//DPAS  bits Live Key 01
EMV_CAPK capk_dpas_live01 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x01,
0x01, 0x01,128,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xB0, 0x80, 0x31, 0xBD, 0xA9, 0xC3, 0x29, 0x1A, 0x3E, 0x8B, 0x9C, 0x79, 0x39, 0x15, 0xF2, 0x47,
0xA3, 0x84, 0x6B, 0x8F}
};


//DPAS  bits Live Key 02
EMV_CAPK capk_dpas_live02 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x02,
0x01, 0x01,160,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xF2, 0x3C, 0xE5, 0xC6, 0x57, 0xF6, 0xEC, 0x44, 0xFA, 0xD8, 0x25, 0x82, 0x35, 0x53, 0x0B, 0xC1,
0x13, 0x75, 0xE9, 0xE1}
};


//DPAS  bits Live Key 03
EMV_CAPK capk_dpas_live03 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x03,
0x01, 0x01, 128,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xB9, 0x6E, 0x38, 0x53, 0x33, 0x77, 0xDD, 0x43, 0x4C, 0x6B, 0x5E, 0xBB, 0xA3, 0x3B, 0x61, 0xDA,
0xD9, 0xA0, 0x4D, 0x51}
};


//DPAS  bits Live Key 04
EMV_CAPK capk_dpas_live04 = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x04,
0x01, 0x01, 176,
{},
1,{0x03},{0x15, 0x12, 0x31},
{0xD9, 0xFD, 0x62, 0xC9, 0xDD, 0x4E, 0x6D, 0xE7, 0x74, 0x1E, 0x9A, 0x17, 0xFB, 0x1F, 0xF2, 0xC5,
0xDB, 0x94, 0x8B, 0xCB}
};


//DPAS  bits Live Key 5A
EMV_CAPK capk_dpas_live5A = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5A,
0x01, 0x01, 128,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0xCC, 0x95, 0x85, 0xE8, 0xE6, 0x37, 0x19, 0x1C, 0x10, 0xFC, 0xEC, 0xB3, 0x2B, 0x5A, 0xE1, 0xB9,
0xD4, 0x10, 0xB5, 0x2D}
};

//DPAS  Live Key 5B
EMV_CAPK capk_dpas_live5B = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5B,
0x01, 0x01, 144,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0x4D, 0xC5, 0xC6, 0xCA, 0xB6, 0xAE, 0x96, 0x97, 0x4D, 0x9D, 0xC8, 0xB2, 0x43, 0x5E, 0x21, 0xF5,
0x26, 0xBC, 0x7A, 0x60}
};


//DPAS  Live Key 5C
EMV_CAPK capk_dpas_live5C = { {0xA0, 0x00, 0x00, 0x01, 0x52}, 0x5C,
0x01, 0x01, 176,
{},
1,{0x03},{0x18, 0x12, 0x31},
{0x60, 0x15, 0x40, 0x98, 0xCB, 0xBA, 0x35, 0x0F, 0x5F, 0x48, 0x6C, 0xA3, 0x10, 0x83, 0xD1, 0xFC,
0x47, 0x4E, 0x31, 0xF8}
};



//EMV应用 用于测试
// EMV applications for test
EMV_APPLIST EMV_TEST_APP = {
	"",
	"\xA0\x00\x00\x00\x99\x90\x90",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x10\x00\x00\x00",
	"\xD8\x40\x04\xF8\x00",
	"\xD8\x40\x00\xA8\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST JCB_TEST_APP = {
	"",
	"\xF1\x23\x45\x67\x89\x01\x23",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x10\x00\x00\x00",
	"\xD8\x40\x04\xF8\x00",
	"\xD8\x40\x00\xA8\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};


EMV_APPLIST JCB_TEST_APP1 = {	// add for JCB testing [7/31/2006 Tommy]
	"",
	"\xA0\x00\x00\x00\x65\x10\x10",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x10\x00\x00\x00",
	"\xFC\x60\xAC\xF8\x00",
	"\xFC\x60\x24\xA8\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x02\x00",
	"",
};

EMV_APPLIST VISA_VSDC_APP = {
	"VISA CREDIT",
	"\xA0\x00\x00\x00\x03\x10\x10",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x10\x00\x00\x00",
	"\xDC\x40\x04\xF8\x00",
	"\xDC\x40\x00\xA8\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x09\x9F\x1A\x02\x95\x05\x9A\x03\x9C\x01",//"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST VISA_ELECTRON_APP = {
	"VISA ELECTRON",
	"\xA0\x00\x00\x00\x03\x20\x10",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x10\x00\x00\x00",
	"\xDC\x40\x04\xF8\x00",
	"\xDC\x40\x00\xA8\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x09\x9F\x1A\x02\x95\x05\x9A\x03\x9C\x01",//"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST VISA_INTERLINK_APP = {
	"VISA INTERLINK",
	"\xA0\x00\x00\x00\x03\x30\x10",
	7,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x00\x00\x00\x00",
	"\xDC\x40\x04\xF8\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x09\x9F\x1A\x02\x95\x05\x9A\x03\x9C\x01",
	//"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST AMEX_TEST_APP = {
	"",
	"\xA0\x00\x00\x00\x25\x01",
	6,PART_MATCH,0,0,0,1,0,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x01",
	"",
};

EMV_APPLIST DPAS_TEST_APP = {
	"",
	"\xA0\x00\x00\x01\x52\x30\x10",  //ApplicationID
	7,PART_MATCH,0,0,0,1,0,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST DPAS_TEST_APP1 = {
	"",
	"\xA0\x00\x00\x03\x24\x10\x10",  //ApplicationID
	7,PART_MATCH,0,0,0,1,0,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST MASTER_MCHIP = {
	"MCHIP",
	"\xA0\x00\x00\x00\x04\x10\x10",
	7,
	PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x04\x00\x00\x00\x00",
	"\xF8\x50\xAC\xF8\x00",
	"\xFC\x50\xAC\xA0\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"\x08\x6C\xF8\x80\x00\x00\x00\x00\x00",
};

EMV_APPLIST MASTER_MAESTRO_APP = {
	"MAESTRO",
	"\xA0\x00\x00\x00\x04\x30\x60",
	7, /*7->5*/
	PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x04\x00\x00\x00\x00",
	"\xF8\x50\xAC\xF8\x00",
	"\xFC\x50\xAC\xA0\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"\x08\x6C\xF8\x80\x00\x00\x00\x00\x00",
};

EMV_APPLIST MASTER_MAESTRO_US_APP = {
	"MAESTRO",
	"\xA0\x00\x00\x00\x04\x22\x03",
	7, /*7->5*/
	PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x04\x00\x00\x00\x00",
	"\xF8\x50\xAC\xF8\x00",
	"\xFC\x50\xAC\xA0\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"\x08\x4C\xF8\x80\x00\x00\x00\x00\x00",
};

EMV_APPLIST MCC_APP4 = {
	"",
	"\xA0\x00\x00\x00\x04\x60\x10",
	7, /*7->5*/
	PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x04\x00\x00\x00\x00",
	"\xF8\x50\xAC\xF8\x00",
	"\xFC\x50\xAC\xA0\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"\x08\x6C\xF8\x80\x00\x00\x00\x00\x00",
};

EMV_APPLIST MCC_APP5 = {
	"",
	"\xA0\x00\x00\x00\x10\x10\x30",
	7, /*7->5*/
	PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x04\x00\x00\x00\x00",
	"\xF8\x50\xAC\xF8\x00",
	"\xFC\x50\xAC\xA0\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"\x08\x6C\xF8\x80\x00\x00\x00\x00\x00",
};

EMV_APPLIST AMEX_LIVE_APP = {
	"","\xA0\x00\x00\x00\x25\x01",
	6,PART_MATCH,0,0,0,1,1,1,
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x01",
	"",
};

EMV_APPLIST DPAS_LIVE_APP = {
	"",
	"\xA0\x00\x00\x01\x52\x30\x10",  //ApplicationID
	6,PART_MATCH,0,0,0,1,0,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x8C",
	"",
};

EMV_APPLIST DPAS_LIVE_APP1 = {
	"",
	"\xA0\x00\x00\x03\x24\x10\x10",  //ApplicationID
	7,PART_MATCH,0,0,0,1,0,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x00\x00",
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x9F\x37\x04",
	"\x00\x8c",
	"",
};


EMV_APPLIST VERVE_APP = {
	"Verve\x00",
	"\xA0\x00\x00\x03\x71\x00\x01",  //ApplicationID
	7,PART_MATCH,0,10,30,1,1,1,	// random=off
	0,0,
	"\x00\x00\x00\x00\x00", //TAC Denial
	"\xDC\x40\x04\xF8\x00", //TAC Online
	"\xDC\x40\x00\xA8\x00", // TAC Default
	"\x00\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x02",
	"",
};


/************************************************
Belows are EMV test data
************************************************/
void InitTestKeys(void)
{
	logTrace(__func__);
	int		i;
	int iRet = 0;

	for (i = 0; i < sizeof(AllTestEmvKey) / sizeof(AllTestEmvKey[0]); i++)
	{
		iRet = EMVAddCAPK(AllTestEmvKey[i].KeyValue);
		if (iRet != EMV_OK) {
			logd(("EMVAddCAPK::%d, iRet = %d, index: %02x", i, iRet, AllTestEmvKey[i].KeyValue->KeyID));
			logHexString("RID ", AllTestEmvKey[i].KeyValue->RID, 5);
		}
	}
}

void InitTestApps(void)
{
	int iRet;
	iRet = EMVAddApp(&EMV_TEST_APP);
	iRet = EMVAddApp(&JCB_TEST_APP);
	iRet = EMVAddApp(&JCB_TEST_APP1);
	iRet = EMVAddApp(&VISA_VSDC_APP);
	iRet = EMVAddApp(&VISA_ELECTRON_APP);
	iRet = EMVAddApp(&AMEX_TEST_APP);
	iRet = EMVAddApp(&DPAS_TEST_APP);
	iRet = EMVAddApp(&DPAS_TEST_APP1);

	iRet = EMVAddApp(&MASTER_MCHIP);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MCHIP::iRet = %d", iRet));
	}
	iRet = EMVAddApp(&MASTER_MAESTRO_APP);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MCHIP::iRet = %d", iRet));
	}

	iRet = EMVAddApp(&MCC_APP4);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MCC_APP4::iRet = %d", iRet));
	}
	iRet = EMVAddApp(&MCC_APP5);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MCC_APP5::iRet = %d", iRet));
	}
	iRet = EMVAddApp(&AMEX_LIVE_APP);
	iRet = EMVAddApp(&DPAS_LIVE_APP);
	iRet = EMVAddApp(&DPAS_LIVE_APP1);
	iRet = EMVAddApp(&VERVE_APP);
}
/************************************************
End of EMV test data
************************************************/






/************************************************
Belows are production EMV data
************************************************/
struct {
	unsigned char KeyID;
	EMV_CAPK *KeyValue;
}AllLiveEmvKey[] = {
	{0x0E,&capk_amex_live67},
	{0x0F,&capk_amex_live0F},
	{0x10,&capk_amex_live10},

	{0x08,&capk_visa_v08},
	{0x09,&capk_visa_v09},
	{0x92,&capk_visa_t92},//for visa paywave test[1/12/2015 jiangjy]

	{0xEF,&capk_mastercard_tEF},
	{0x05,&capk_mastercard_v05},
	{0x06,&capk_mastercard_v06},

	{0x01,&capk_dpas_live01},//for DPAS test[2015/12/17 Gillian]
	{0x02,&capk_dpas_live02},
	{0x03,&capk_dpas_live03},
	{0x04,&capk_dpas_live04},
	{0x5A,&capk_dpas_live5A},
	{0x5B,&capk_dpas_live5B},
	{0x5C,&capk_dpas_live5C},

	{0x05, &verve_capk_v05},
	{0x06, &verve_capk_v06}
};

void InitLiveKeys(void)
{
	logTrace(__func__);
	int		i, iRet;

	for (i = 0; i < sizeof(AllLiveEmvKey) / sizeof(AllLiveEmvKey[0]); i++)
	{
		iRet = EMVAddCAPK(AllLiveEmvKey[i].KeyValue);
		if (iRet != EMV_OK) {
			logd(("EMVAddCAPK::%d, iRet = %d, index: %02x", i, iRet, AllLiveEmvKey[i].KeyValue->KeyID));
			logHexString("RID ", AllLiveEmvKey[i].KeyValue->RID, 5);
		}
	}
}

void InitLiveApps(void)
{
	int iRet;

	iRet = EMVAddApp(&VISA_VSDC_APP);
	iRet = EMVAddApp(&VISA_ELECTRON_APP);
	iRet = EMVAddApp(&MASTER_MCHIP);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MCHIP::iRet = %d", iRet));
	}
	iRet = EMVAddApp(&MASTER_MAESTRO_APP);
	if (iRet != EMV_OK) {
		logd(("EMVAddApp::MASTER_MAESTRO_APP::iRet = %d", iRet));
	}

	iRet = EMVAddApp(&AMEX_LIVE_APP);
	iRet = EMVAddApp(&DPAS_LIVE_APP);
	iRet = EMVAddApp(&DPAS_TEST_APP1);
	iRet = EMVAddApp(&VERVE_APP);
}

// end of file
