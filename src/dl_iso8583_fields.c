...
DL_ERR _pack_iso_ASCII_ASC ( DL_UINT16                    iField,
						 const DL_ISO8583_MSG        *iMsg,
						 const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
						 DL_UINT8                   **ioPtr );

DL_ERR _unpack_iso_ASCII_ASC ( DL_UINT16                    iField,
						   DL_ISO8583_MSG              *ioMsg,
						   const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
						   DL_UINT8                   **ioPtr );

DL_ERR _pack_iso_BITMAP_ASC ( DL_UINT16                    iField,
						  const DL_ISO8583_MSG        *iMsg,
						  const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
						  DL_UINT8                   **ioPtr );

DL_ERR _unpack_iso_BITMAP_ASC ( DL_UINT16                    iField,
						    DL_ISO8583_MSG              *ioMsg,
						    const DL_ISO8583_FIELD_DEF  *iFieldDefPtr,
						    DL_UINT8                   **ioPtr );
...
static DL_ERR VarLen_PutASC ( DL_UINT8    iVarLenType,
						   DL_UINT32   iActLen,
						   DL_UINT32  *ioReqLen,
						   DL_UINT8  **ioPtr );

static DL_ERR VarLen_GetASC ( const DL_UINT8 **ioPtr,
...
static DL_ISO8583_TYPE fieldTypeArr_Nibss[] = {
/* ISO_N    */ {_unpack_iso_ASCHEX,_pack_iso_ASCHEX},
/* ISO_NS   */ {_unpack_iso_BINARY,_pack_iso_BINARY},
/* ISO_XN   */ {_unpack_iso_ASCHEX,_pack_iso_ASCHEX},
/* ISO_A    */ {_unpack_iso_ASCII_ASC ,_pack_iso_ASCII_ASC },
/* ISO_AN   */ {_unpack_iso_ASCII_ASC ,_pack_iso_ASCII_ASC },
/* ISO_ANS  */ {_unpack_iso_ASCII_ASC ,_pack_iso_ASCII_ASC },
/* ISO_ANSB */ {_unpack_iso_ASCII_ASC ,_pack_iso_ASCII_ASC },
/* ISO_ANP  */ {_unpack_iso_ASCII_ASC ,_pack_iso_ASCII_ASC },
/* ISO_B    */ {_unpack_iso_BINARY,_pack_iso_BINARY},
/* ISO_Z    */ {_unpack_iso_BINARY,_pack_iso_BINARY},
/* ISO_BMAP */ {_unpack_iso_BITMAP_ASC,_pack_iso_BITMAP_ASC}
};
...
//#define GetFieldType(fieldType)\       //disabled for the NIBSS mod
// (&fieldTypeArr[fieldType])

// gets the field type details
#define GetFieldType(fieldType)\
 (&fieldTypeArr_Nibss[fieldType])
 ...
 for ( i=0 ; i<((kDL_ISO8583_MAX_FIELD_IDX-iField+1)+63)/64 ; i++ )
 {
  ...
  else
  {
   if ( ms || ls )
   {
    ...
    hex2bin(mytemp,myBin,2);
    myBin[0] |= 0x80;

    //HexEnCodeMethod(myBin,1,myHex);
    binTohex(myBin,myHex,1);
					        ...
					        DL_UINT8                   **ioPtr )
{
...

	{
		...
		do
		{
			...
			hex2bin(tmpPtr,temp,8);
			/* get bitmap segment (8 bytes) */
			ms = DL_BYTES_TO_UINT32(temp);
			ls = DL_BYTES_TO_UINT32(temp+4);
			tmpPtr += 16;//8;

			/*if(iFieldDefPtr->fieldType==kDL_ISO8583_BMP){
				//hex2bin(tmpPtr,temp,8);
				// get bitmap segment (8 bytes)
				ms = DL_BYTES_TO_UINT32(temp);
				ls = DL_BYTES_TO_UINT32(temp+4);
	...
	switch ( iVarLenType )
	{
		...
		case kDL_ISO8583_LLVAR:
			...
			*ioReqLen  = iActLen;
			...
			snprintf(format, 10, "%%0%dd", iVarLenType);
			sprintf(tmpPtr, format, *ioReqLen);
			tmpPtr+=iVarLenType;
			break;
		case kDL_ISO8583_LLLVAR:
			iActLen   %= 1000;
	...
	*oLen = iMaxValue;
	...
	if ( kDL_ISO8583_FIXED != iVarLenDigits )
	{
		*oLen = 0;
		...
		char lenString[5] = "\0";
		strncpy(lenString, tmpPtr, iVarLenDigits);
		*oLen =  atoi(lenString);
		tmpPtr+=iVarLenDigits;
						 ...
						 DL_UINT8                   **ioPtr )
{
	...
	/* variable length handling */
	err = VarLen_PutASC(iFieldDefPtr->varLen,actLen,&reqLen,&tmpPtr);
						   ...
						   DL_UINT8                   **ioPtr )
{
	...
	/* variable length handling */
	err = VarLen_GetASC(&tmpPtr,iFieldDefPtr->varLen,iFieldDefPtr->len,&size);
